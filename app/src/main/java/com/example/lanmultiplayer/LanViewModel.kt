...
class LanViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private fun handleMessage(t: String) {
        ...
        if (player.firstOrNull()=="PLAYER" && player.getOrNull(1)=="JOIN") { val name=player.getOrNull(3) ?: return; _match.update { it.copy(players=(it.players + name).distinct(), opponentName=if(it.opponentName=="对手") name else it.opponentName, message="$name 已加入房间") }; return }
