...
class LanViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private fun handleMessage(t: String) {
        ...
        if (player.firstOrNull()=="PLAYER") { val action=player.getOrNull(1); val name=player.getOrNull(3) ?: return; if(action=="JOIN") _match.update { it.copy(players=(it.players + name).distinct(), opponentName=if(it.opponentName=="对手") name else it.opponentName, message="$name 已加入房间") }; if(action=="LEAVE") _match.update { it.copy(players=it.players.filterNot { n -> n==name }, opponentName=if(it.opponentName==name) "对手" else it.opponentName, message="$name 已离开房间") }; return }
